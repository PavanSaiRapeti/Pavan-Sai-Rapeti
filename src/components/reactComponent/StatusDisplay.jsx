...
const StatusDisplay = React.memo(function StatusDisplay() {
  ...
  return (
    <div className="status-display-root h-full min-h-0 text-center flex flex-col justify-center px-2 py-3 md:px-6 md:py-4 overflow-hidden">
      <p className="status-availability text-gray-700 pt-1 md:pt-10 text-[clamp(0.7rem,2.8vw,0.8rem)] shrink-0">
        {staticText.statusDisplay.availabilityLabel}
      </p>
      <h2 className="status-role-heading font-logo leading-[1.05] mt-1.5 md:mt-4 px-1 break-words shrink-0">
        <span className="status-intro">{staticText.statusDisplay.introPrefix}</span>{" "}
        <span className="status-role-slot text-black underline decoration-2 underline-offset-4">
          {role}
        </span>
      </h2>
      <div className="status-body-block text-gray-700 pt-4 md:pt-10 text-center max-w-2xl mx-auto min-h-0 overflow-hidden">
